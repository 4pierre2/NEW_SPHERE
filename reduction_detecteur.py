...
def unshear(im,mat, order=3):
    return scipy.ndimage.interpolation.affine_transform(im, mat, order=order)
...
def unshear_cube(cube,mat, order=3):
    new_cube=np.copy(cube)
    i=0
    for im in cube:
        new_cube[i]=unshear(im,mat, order=order)
        i=i+1
    return new_cube
...
obj_std = obj_ims_realigned.std(0)/np.sum(~obj_ims_realigned.mask, 0)**0.5
...
calstar_std = calstar_ims_realigned.std(0)/np.sum(~calstar_ims_realigned.mask, 0)**0.5
...
obj = unshear(obj, matrix_shear, order=1)
obj_std = unshear(obj_std, matrix_shear, order=1)
calstar = unshear(calstar, matrix_shear, order=1)
calstar_std = unshear(calstar_std, matrix_shear, order=1)
wave = unshear(wave, matrix_shear, order=1)
